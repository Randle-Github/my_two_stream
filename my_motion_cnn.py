...
class Motion_CNN():
    def __init__(self, lr):
        self.lr = lr
        self.model = resnet101(pretrained=True, channel=6).cuda()
...
if __name__ == '__main__':
    model = Motion_CNN(0.01)
    file_name = ["testlist01", "testlist02", "testlist03", "trainlist01", "trainlist02", "trainlist03"]
    dataloader1 = motion_dataloader(file_name[0], 64)
    dataloader2 = motion_dataloader(file_name[1], 64)
    dataloader3 = motion_dataloader(file_name[2], 64)
    dataloader4 = motion_dataloader(file_name[3], 1)
    dataloader5 = motion_dataloader(file_name[4], 1)
    dataloader6 = motion_dataloader(file_name[5], 1)
    for i in range(5000):
    ...
    np.save("/opt/data/private/my_two_stream/testlist01_motion_pred.npy", np.array(pred4))
    np.save("/opt/data/private/my_two_stream/testlist02_motion_pred.npy", np.array(pred5))
    np.save("/opt/data/private/my_two_stream/testlist03_motion_pred.npy", np.array(pred6))
